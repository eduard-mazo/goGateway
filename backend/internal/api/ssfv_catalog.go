...
func (h *SSFVHandler) Mount(r chi.Router) {
	...
	r.Get("/host/nodes", h.listHostNodes)
	r.Get("/host/metrics", h.listHostMetrics)
	r.Get("/host/ifaces", h.listHostIfaces)
	r.Post("/host/ifaces", h.createHostIface)
	r.Put("/host/ifaces/{id}", h.updateHostIface)
	r.Delete("/host/ifaces/{id}", h.deleteHostIface)
...
// ─── Telemetría de host (System/* del nodo gateway) ───────────────────────────

// listHostNodes returns the distinct gateway nodes that have reported host
// telemetry, with how many distinct metrics each has and when last seen.
func (h *SSFVHandler) listHostNodes(w http.ResponseWriter, r *http.Request) {
	pool := h.pool()
	if pool == nil {
		errResp(w, http.StatusServiceUnavailable, "ssfv adapter not connected")
		return
	...
	defer cancel()
	rows, err := pool.Query(ctx, `
		SELECT node_topic, COUNT(*) AS metricas, MAX(timestamp_utc) AS last_seen
		FROM ssfv.v_host_ultimas
		GROUP BY node_topic
		ORDER BY node_topic`)
	if err != nil {
		errResp(w, http.StatusInternalServerError, err.Error())
	...
	for rows.Next() {
		...
		var n int
		var last time.Time
		if err := rows.Scan(&node, &n, &last); err != nil {
			continue
		}
		out = append(out, map[string]any{"node_topic": node, "metricas": n, "last_seen": last})
	}
	jsonResp(w, http.StatusOK, out)
}

// listHostMetrics returns the latest value of every host metric, optionally
// filtered to one node via ?node=. Grouped client-side by categoria/subkey.
func (h *SSFVHandler) listHostMetrics(w http.ResponseWriter, r *http.Request) {
	pool := h.pool()
	if pool == nil {
		errResp(w, http.StatusServiceUnavailable, "ssfv adapter not connected")
		return
	...
	query := `
		SELECT node_topic, categoria, subkey, metrica, valor, calidad, timestamp_utc
		FROM ssfv.v_host_ultimas`
	args := []any{}
	if node != "" {
		query += ` WHERE node_topic = $1`
		args = append(args, node)
	}
	query += ` ORDER BY categoria, subkey, metrica`
	...
	out := []map[string]any{}
	for rows.Next() {
		var nodeTopic, categoria, subkey, metrica, calidad string
		var valor *float64
		var ts time.Time
		if err := rows.Scan(&nodeTopic, &categoria, &subkey, &metrica, &valor, &calidad, &ts); err != nil {
			continue
		}
		out = append(out, map[string]any{
			"node_topic": nodeTopic, "categoria": categoria, "subkey": subkey,
			"metrica": metrica, "valor": valor, "calidad": calidad, "timestamp_utc": ts,
		})
	}
	jsonResp(w, http.StatusOK, out)
}

func (h *SSFVHandler) listHostIfaces(w http.ResponseWriter, r *http.Request) {
	pool := h.pool()
	if pool == nil {
		errResp(w, http.StatusServiceUnavailable, "ssfv adapter not connected")
		return
	...
	defer cancel()
	rows, err := pool.Query(ctx, `
		SELECT iface_id, nombre, descripcion, activo
		FROM ssfv.cfg_host_iface ORDER BY nombre`)
	if err != nil {
		errResp(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	for rows.Next() {
		...
		var descripcion *string
		var activo bool
		if err := rows.Scan(&id, &nombre, &descripcion, &activo); err != nil {
			continue
		}
		out = append(out, map[string]any{
			"iface_id": id, "nombre": nombre, "descripcion": descripcion, "activo": activo,
		})
	}
	jsonResp(w, http.StatusOK, out)
}

func (h *SSFVHandler) createHostIface(w http.ResponseWriter, r *http.Request) {
	pool := h.pool()
	if pool == nil {
		errResp(w, http.StatusServiceUnavailable, "ssfv adapter not connected")
		return
	...
	if body.Nombre == "" {
		errResp(w, http.StatusBadRequest, "nombre requerido")
		return
	}
	activo := true
	if body.Activo != nil {
	...
	if err := pool.QueryRow(ctx, `
		INSERT INTO ssfv.cfg_host_iface (nombre, descripcion, activo)
		VALUES ($1,$2,$3)
		ON CONFLICT (nombre) DO UPDATE SET descripcion = EXCLUDED.descripcion, activo = EXCLUDED.activo
		RETURNING iface_id`, body.Nombre, body.Descripcion, activo).Scan(&id); err != nil {
		errResp(w, http.StatusInternalServerError, err.Error())
		return
	}
	h.triggerReload()
	jsonResp(w, http.StatusCreated, map[string]any{"iface_id": id})
}

func (h *SSFVHandler) updateHostIface(w http.ResponseWriter, r *http.Request) {
	id, _ := strconv.Atoi(chi.URLParam(r, "id"))
	pool := h.pool()
	if pool == nil {
		errResp(w, http.StatusServiceUnavailable, "ssfv adapter not connected")
	...
	if _, err := pool.Exec(ctx, `
		UPDATE ssfv.cfg_host_iface SET nombre=$1, descripcion=$2, activo=$3 WHERE iface_id=$4`,
...
func (h *SSFVHandler) deleteHostIface(w http.ResponseWriter, r *http.Request) {
	h.deleteRow(w, r, `DELETE FROM ssfv.cfg_host_iface WHERE iface_id=$1`)
	h.triggerReload()
}
