...
const mapCols = `id,server_id,topic_id,device_name,variable_type,characteristic,json_key,quality_key,metric_name,iec104_type,ioa,unit,scale,enabled,business,company`
...
func (h *MappingHandler) create(w http.ResponseWriter, r *http.Request) {
	...
	res, err := h.DB.Exec(`INSERT INTO signal_mappings(server_id,topic_id,device_name,variable_type,characteristic,json_key,quality_key,metric_name,iec104_type,ioa,unit,scale,enabled,business,company) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		m.ServerID, m.TopicID, m.DeviceName, m.VariableType, m.Characteristic, m.JSONKey, m.QualityKey, m.MetricName, m.IEC104Type, m.IOA, m.Unit, m.Scale, m.Enabled, m.Business, m.Company)
	if err != nil {
		writeErr(w, 400, err.Error())
		return
	}
...
func (h *MappingHandler) update(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := h.DB.Exec(`UPDATE signal_mappings SET server_id=?,topic_id=?,device_name=?,variable_type=?,characteristic=?,json_key=?,quality_key=?,metric_name=?,iec104_type=?,ioa=?,unit=?,scale=?,enabled=?,business=?,company=? WHERE id=?`,
		m.ServerID, m.TopicID, m.DeviceName, m.VariableType, m.Characteristic, m.JSONKey, m.QualityKey, m.MetricName, m.IEC104Type, m.IOA, m.Unit, m.Scale, m.Enabled, m.Business, m.Company, id); err != nil {
		writeErr(w, 400, err.Error())
		return
	}
	m.ID = id
